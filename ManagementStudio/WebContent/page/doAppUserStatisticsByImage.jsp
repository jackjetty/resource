...
<body style="background: #DFE8F6;">
		...
		<table height="100%">
			<tr	style="Valign: center; text-decoration: none; line-heigth: 30px; height: 30px; margin-left: 50px;">
				...
				<option value="2">按月</option><option value="3">按周</option><option value="4">按日</option></select></td>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#exchange").click(function() {
			$("#form1").attr('action','doAppUserStatisticsAll.action');
			$("#form1").submit();
		});
		...
		$("#search").click(function(){
			...
			var endTime = $("#endTime").datebox('getValue');
			var startTime = $("#startTime").datebox('getValue');
			var time1=startTime.split("-")[0]+startTime.split("-")[1]+startTime.split("-")[2];
			var time2=endTime.split("-")[0]+endTime.split("-")[1]+endTime.split("-")[2];
			var xiangcha=parseInt(time2)-parseInt(time1);
			if(xiangcha<=0){
				alert("结束时间必须大于开始时间");
				return false;
			}
			if(toTime=='1'){
				var tt=parseInt(time2)-parseInt(time1);
				if(tt<1000){
					alert("请保证时间相隔一年以上");
					return false;
				}
			...
			if(toTime=='2'){
				...
				var month=12*(parseInt(endTime.split("-")[0])-parseInt(startTime.split("-")[0]))+parseInt(endTime.split("-")[1])-parseInt(startTime.split("-")[1]);
				if(month>12){
					alert("按月查询时间相隔不能超过一年");
					return false;
				}
			}
			if(toTime=='3'){
				var tt=DateDiff(startTime,endTime);
				if(tt<7){
					alert("请保证时间相隔一周以上");
					return false;
				}
	...
	function tubiao(placeName,typeLeiXing,toTime,startTime,endTime){
		var titleText=startTime+'至'+endTime+''+placeName+'APP用户统计信息';
		$.ajax({
			url : "getAllAppUserInfo.action",
			async : false,
			type : "post",
			...
			success : function(data) {
				...
				var allshuju1=eval(data.allshuju);
				$(document).ready(function() {
					new Highcharts.Chart({
						...
						yAxis: {
			                ...
			                filename:'chart',  
			                url:'toDownloadByImage.action?banben='+banben
			            },  
						legend: {
							layout: 'horizontal',
