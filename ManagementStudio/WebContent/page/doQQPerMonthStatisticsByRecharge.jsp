...
<body style="background: #DFE8F6;">
		...
		<table height="100%">
			<tr	style="Valign: center; text-decoration: none; line-heigth: 30px; height: 30px; margin-left: 50px;">
				...
				<option value="2">按月</option><option value="3">按周</option></select></td>
...
<script type="text/javascript">
	...
	$(function() {
		...
		tubiao('column','3',startTime,endTime);
		$("#search").click(function(){
			...
			tubiao(typeLeiXing,toTime,startTime,endTime);
		});
	...
	function tubiao(typeLeiXing,toTime,startTime,endTime){
		var titleText=startTime+'至'+endTime+'Q币包月统计信息';
		$.ajax({
			url : "getQQStatistics.action",
			async : false,
			type : "post",
			data : "startTime="+startTime+"&endTime="+endTime+"&toTime="+toTime,
			dataType : "json",
			success : function(data) {
				var xAxis1=data.xAxis.split(",");
				var newshuju1=eval(data.newshuju1);
				var newshuju2=eval(data.newshuju2);
				var newshuju3=eval(data.newshuju3);
