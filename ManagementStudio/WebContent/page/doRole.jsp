...
<body style="background: #DFE8F6;">
	...
	<div id="searchpanel" class="easyui-panel panel-margin-buttom" title="用户角色查询"
		style="height: 80px; padding: 10px;">
		<div style="float:left;vertical-align: center;height:31px;line-height:31px;">
		角色名称：<input type="text" id="name" /> </div>
	...
	<table id="dg" class="easyui-datagrid" title="用户角色列表"
		...
		<thead>
			<tr>
				...
				<!-- th data-options="field:'id',width:200">角色ID</th> -->
				<th data-options="field:'name',width:300,sortable:true,align:'center'">角色名称</th>
				<th data-options="field:'status',width:200,sortable:true">角色状态</th>
				<th data-options="field:'describe',width:200">角色描述</th>
			</tr>
		</thead>
	</table>
		...
		var toolbar = [ {
			...
			handler : function() {
				...
				if (rows.length == 0) {
					...
					var id = row.id;
					$("#editRoleId").val(id);
					var url = "getAllMenu.action?roleId=" + id;
					$('#tree').tree({
						url : url
					});
				}
		...
		$(function() {
			...
			$("#addRole").click(
					function() {
						...
						var describe = $("#add_describe").val().trim();
						$.ajax({
							url : "addRole.action",
			...
			$("#updateRole").click(
					function() {
						...
						var describe = $("#edit_describe").val().trim();
						$.ajax({
	...
	<div id="dlg" class="easyui-dialog" title="添加用户角色"
		...
		<div>
			<ul style="list-style: none;">
				...
				<li style="line-height: 40px; height: 40px;">角色描述：<input
					type="text" id="add_describe" /><font color="red">&nbsp;&nbsp;*&nbsp;&nbsp;选填</font></li>
			</ul>
		</div>
		<div align="center">
			...
			<ul style="list-style: none;">
				...
				<li style="line-height: 40px; height: 40px;">角色状态：<select
					id="edit_status" style="width: 155px;"><option value="未启用">未启用</option>
						...
						<option value="停用">停用</option></select><font color="red">&nbsp;&nbsp;*&nbsp;&nbsp;必填</font></li>
				<li style="line-height: 40px; height: 40px;">角色描述：<input
					type="text" id="edit_describe" /><font color="red">&nbsp;&nbsp;*&nbsp;&nbsp;选填</font></li>
			</ul>
		</div>
		<div align="center">
