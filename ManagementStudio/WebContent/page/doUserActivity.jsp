...
<body style="background: #DFE8F6;">
		...
		title="用户活跃度查询" style="height: 80px; padding: 10px;overflow: hidden;">
...
<script type="text/javascript">
	...
	function tubiao(placeName,typeLeiXing,toTime,startTime,endTime){
		var titleText=startTime+'至'+endTime+''+placeName+'用户活跃度统计信息';
		$.ajax({
			url : "getUserActivity.action",
			async : false,
			type : "post",
			...
			success : function(data) {
				...
				var successOrderNumbers=eval(data.successOrderNumbers);
				var failOrderNumbers=eval(data.failOrderNumbers);
				var AllOrderNumbers=eval(data.AllOrderNumbers);
				$(document).ready(function() {
					new Highcharts.Chart({
						...
						plotOptions: {
					        ...
					        series: [{
					        	name: '交易用户个数',
								data: AllOrderNumbers
					        },{
							name: '成功交易笔数',
							data: successOrderNumbers
						},{
							name: '失败交易笔数',
							data: failOrderNumbers
						}]
					});
