...
@Service("workGuideProcessService")
public class WorkGuideProcessServiceImpl   extends ProcessServiceImpl{
	...
	public boolean clickEventProcess(ClickEventReqBean clickEventReqBean){ 
		...
		boolean sign=true;
		sign=eventKey.equalsIgnoreCase(Constant.WORKGUIDE)?sign:false;
		if(!sign) return false;  
		if(!processDao.isUsing(Constant.WORKGUIDE)){
			this.hopeMessage();
			return true;
		}
