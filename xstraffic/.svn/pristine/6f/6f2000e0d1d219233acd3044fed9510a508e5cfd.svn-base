...
public class QRWapServiceImpl implements QRWapService {
	...
	public HashMap<String, Object> win( ) {
		...
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		String hql="from TbWeiUser ";
		List<TbWeiUser> weiUserList=weiUserDao.findPage(hql, 0, 1, new ArrayList());
		
		TbWeiUser tbWeiUser=weiUserList.get(0);
		tbWeiUser.setPhoneNumber("");
		weiUserDao.saveOrUpdate(tbWeiUser);
		System.out.println("tbWeiUser name : "+tbWeiUser.getNickname());
		tbWeiUser=new TbWeiUser();
		tbWeiUser.setOpenId("o5thVuBVqqubdwAGSIpoOnDk2QW0"); 
		weiUserDao.save(tbWeiUser);
		resultMap.put("weiuser",  tbWeiUser);
//o5thVuB2lbphtXI-sRDIrZyPeCss
		return resultMap;
	}
